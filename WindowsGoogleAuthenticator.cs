...
public class WindowsGoogleAuthenticator : NetworkAuthenticator
{
    [Header("Google Authentication Settings")] [Space(20f)]
    ...
    [Header("Scopes")] public List<string> googleScopes = new List<string>();
        ...
        if (msg.authority == "Google")
        {
            ...
            if (response.StartsWith("SUCCESS"))
            {
                ...
                if (UUID == "invalid_uuid")
                {
                    ...
                    message =
                        "Authentication Invalidated: Failed to retrieve the user's UUID from the authentication process.";
                    AuthResponseMessage invalidatedResponse =
                        new AuthResponseMessage(AuthenticationResponseCode.Failed, message);
                ...
                AuthResponseMessage authResponseMessage =
                    new AuthResponseMessage(AuthenticationResponseCode.Success, message);
            ...
            try
            {
                ...
                using (StreamReader reader = new StreamReader(tokenResponse.GetResponseStream()))
                {
                    ...
                    return Task.Factory.StartNew(() =>
                        "FAILED{\"error\": \"null_token_endpoint\",\"error_description\": " +
    ...
    private async Task<string> GetUserInfoFromProvider(string accessToken, string authority)
    {
        ...
        return
            "FAILED{\"error\": \"invalid_authority\",\"error_description\": \"Client provided authentication using an invalid authority.\"}";
    }
    ...
    public async Task<AuthenticationResponseCode> SendAuthRequestMessage()
    {
        ...
        Debug.Log(
            "[Authentication][Client] Successfully received authentication code from the authentication provider.");
    ...
    private async Task<string> GetAuthorizationCodeWindows()
    {
        switch (provider)
        {
            case "Google":
                ...
                {
                    Debug.LogError(
                        $"[Authentication][Client] OAuth authorization error: {context.Request.QueryString.Get("error")}.");
                ...
                {
                    Debug.LogError(
                        "[Authentication][Client] [Authentication][Client] Malformed authorization response. " +
    ...
    private HttpListener StartHttpListener()
    {
        ...
        http.Start();
        Debug.Log(
            "[Authentication][Client] Successfully started listening for authentication loopback using HTTPListener.");
        return http;
    }
